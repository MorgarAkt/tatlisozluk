document.addEventListener("DOMContentLoaded", function () {
        ...
        if (query.trim() !== "") {
            try {
                var response = await fetch(
                    "/title/search?query=" + encodeURIComponent(query)
                );
    ...
    searchButton.addEventListener("click", () => {
        ...
        if (query.trim() !== "" && searchResults.children.length > 0) {
            ...
            div[0].appendChild(paragraphElement);
            var pElement = div[0].querySelector("p");
            pElement.textContent = "Lütfen önce aramaya bir şeyler yazın!";
